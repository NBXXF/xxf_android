...
import com.alibaba.android.arouter.config.ARouterConfig;
    ...
    public Postcard build(String path) {
        ...
        try {
            ...
            if (Objects.equals("http", scheme) || Objects.equals("https", scheme)) {
                Postcard buildPostcard = _ARouter.getInstance().build(path);
                buildPostcard.withString(ARouterConfig.KEY_WEB_URL, path);
            ...
            if (!TextUtils.isEmpty(query)) {
                String newPath = path.replace(String.format("?%s", query), "");
                Postcard buildPostcard = _ARouter.getInstance().build(newPath);
                Set<String> paramKeys = parse.getQueryParameterNames();
                if (paramKeys != null) {
                    /**
                     * 参数
                     */
                    ...
                    for (String key : paramKeys) {
                        paramBundle.putString(key, parse.getQueryParameter(key));
                    }
                    buildPostcard.with(paramBundle);
                }
                return buildPostcard;
            }
        } catch (Throwable e) {
