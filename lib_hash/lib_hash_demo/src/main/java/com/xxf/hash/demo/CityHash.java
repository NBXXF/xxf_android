...
public class CityHash {
	...
	private static long[] weakHashLen32WithSeeds(long w, long x, long y,
	...
	public static long[] cityHash128(byte[] s, int pos, int len) {
		...
		if (len >= 16) {
			...
			return cityHash128WithSeed(new byte[0], 0, 0, fetch64(s, pos + 0)
					^ (len * k0), fetch64(s, pos + len - 8) ^ k1);
		} else {
	...
	public static void main(String[] arg) {
		...
		long t = CityHash
				.cityHash64("1a".getBytes(), 0, "1a".getBytes().length);
		System.out.println(t);
		t = CityHash.cityHash64("2a".getBytes(), 0, "2a".getBytes().length);
		System.out.println(t);
		t = CityHash.cityHash64("3a".getBytes(), 0, "3a".getBytes().length);
		System.out.println(t);
