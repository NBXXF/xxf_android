...
`xxf_multi_language` 是项目内的应用内多语言模块，主包名为 `com.xxf.android.multilanguage`。它基于 AndroidX AppCompat 的 per-app language 能力，额外补充了语言缓存、启动恢复、全局资源读取和 HTTP `Accept-Language` 生成。

## 1. 结论

普通业务接入时只需要关注三件事：

1. 业务模块显式依赖 `xxf_multi_language`。
...
```kotlin
import com.xxf.android.multilanguage.getLocalizedString
import com.xxf.android.multilanguage.restoreCachedAppLanguage
import com.xxf.android.multilanguage.setAppLanguage
```
...
| 方案 | 优点 | 缺点 | 适用场景 |
| --- | --- | --- | --- |
| `xxf_multi_language` 当前封装 | 基于 AppCompat 官方兼容能力；支持 Android 12 及以下应用内切换；有 `SharedPreferencesOwner` 缓存；提供全局资源 `getLocalizedXxx(...)`；文档和 API 收敛在独立模块 | 业务需要显式依赖新模块；启动早期要调用 `restoreCachedAppLanguage()`；非 UI 全局资源仍需要改用封装 API | 当前项目推荐方案 |
...
- 旧版 `MultiLanguage.kt` 已从 `lib_ktx` 移到独立模块，业务需要显式依赖 `xxf_multi_language`。
- `lib_ktx` 没有继续 `api` 导出本模块，因为会形成依赖环：`xxf_multi_language -> lib_preferences -> lib_application -> lib_ktx -> xxf_multi_language`。
