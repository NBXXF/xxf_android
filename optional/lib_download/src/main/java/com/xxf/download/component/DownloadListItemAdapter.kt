 ...
 * Date: 1/4/19 6:05 PM
 * Description: 下载队列更新高度抽象ListAdapter
 */
abstract class DownloadListItemAdapter<V : ViewBinding, T : IDownloadEntity> :
    XXFRecyclerListAdapter<V, T> {
    ...
    private val mDownloadListener: DownloadUpdateListener<T> =
        object : DownloadUpdateListener<T>() {
            override fun updateDownload(task: T?, info: DownloadInfo) {
                runOnUiThread {
                    this@DownloadListItemAdapter.updateDownload(task, info)
                }
            }
        }
