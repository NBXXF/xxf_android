...
public interface JBinderFactory {
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            if (code == IBinder.FIRST_CALL_TRANSACTION) {
                try {
                    ...
                    if (magicNumber != JBinderVersion.MAGIC_NUMBER) {
                        throw new IllegalArgumentException("Mismatched magic number " + magicNumber);
                    }
                    int version = data.readInt();
                    if (!JBinderVersion.isSupported(version)) {
                        throw new IllegalArgumentException("Unsupported version " + version +
                                ", the min supported is " + JBinderVersion.minSupported());
                    }
                    String functionId = data.readString();
                    int argCount = data.readInt();
                    Object[] args = new Object[argCount];
                    for (int i = 0; i < argCount; i++) {
                        args[i] = JBinderParcel.read(data, classLoader);
                    }
                    ...
                    if (reply != null) {
                        ...
                        if (result != null) {
                            reply.writeInt(1);
                            JBinderParcel.write(reply, result);
                        } else {
                            reply.writeInt(0);
                        }
                    }
                    ...
                    Log.e(JBinder.TAG, "Binder call failed", cause);
    ...
    class BaseProxy {
        ...
        protected Object transact(int flags, String methodId, Object... args) {
            ...
            try {
                ...
                data.writeInt(JBinderVersion.MAGIC_NUMBER);
                data.writeInt(JBinderVersion.current());
                data.writeString(methodId);
                if (args != null) {
                    data.writeInt(args.length);
                    for (Object arg : args) {
                        JBinderParcel.write(data, arg);
                    }
                } else {
                    data.writeInt(0);
                }
                ...
                reply.readException();
                if (reply.readInt() != 0) {
                    result = JBinderParcel.read(reply, classLoader);
                }
            } catch (Throwable e) {
