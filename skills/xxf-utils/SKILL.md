---
name: xxf-utils
description: :lib_utils 模块规则（core library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---
...
- Gradle path: `:lib_utils`
- Directory: `lib_utils`
- Status: enabled in settings.gradle
- Type: core library
- Namespace: `com.xxf.utils`
...
- 优先运行 `./gradlew :lib_utils:compileDebugKotlin`；任务不存在时退回 `./gradlew :lib_utils:assembleDebug`。
