---
name: xxf-blockcanary-blockcanary-analyzer
description: :optional:blockcanary:blockcanary-analyzer 模块规则（optional library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---
...
- Gradle path: `:optional:blockcanary:blockcanary-analyzer`
- Directory: `optional/blockcanary/blockcanary-analyzer`
- Status: enabled in settings.gradle
- Type: optional library
- Namespace: `com.github.moduth.blockcanary.core`
...
- 优先运行 `./gradlew :optional:blockcanary:blockcanary-analyzer:compileDebugKotlin`；任务不存在时退回 `./gradlew :optional:blockcanary:blockcanary-analyzer:assembleDebug`。
