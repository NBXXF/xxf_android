---
name: xxf-blockcanary-blockcanary-android-no-op
description: :optional:blockcanary:blockcanary-android-no-op 模块规则（optional library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---

# :optional:blockcanary:blockcanary-android-no-op

## Scope

- Gradle path: `:optional:blockcanary:blockcanary-android-no-op`
- Directory: `optional/blockcanary/blockcanary-android-no-op`
...
- 优先运行 `./gradlew :optional:blockcanary:blockcanary-android-no-op:compileDebugKotlin`；任务不存在时退回 `./gradlew :optional:blockcanary:blockcanary-android-no-op:assembleDebug`。
