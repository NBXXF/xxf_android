---
name: xxf-fileprovider
description: :lib_fileprovider 模块规则（core library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---
...
- Gradle path: `:lib_fileprovider`
- Directory: `lib_fileprovider`
- Status: enabled in settings.gradle
- Type: core library
- Namespace: `com.xxf.fileprovider`
...
- 优先运行 `./gradlew :lib_fileprovider:compileDebugKotlin`；任务不存在时退回 `./gradlew :lib_fileprovider:assembleDebug`。
