---
name: xxf-profileinstaller
description: :optional:lib_profileinstaller 模块规则（optional library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---

# :optional:lib_profileinstaller

## Scope

- Gradle path: `:optional:lib_profileinstaller`
- Directory: `optional/lib_profileinstaller`
- Status: enabled in settings.gradle
- Type: optional library
- Namespace: `com.xxf.profile.installer`
...
- 优先运行 `./gradlew :optional:lib_profileinstaller:compileDebugKotlin`；任务不存在时退回 `./gradlew :optional:lib_profileinstaller:assembleDebug`。
