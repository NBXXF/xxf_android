---
name: xxf-view-gradient
description: :lib_view_gradient 模块规则（core library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---
...
- Gradle path: `:lib_view_gradient`
- Directory: `lib_view_gradient`
- Status: enabled in settings.gradle
- Type: core library
- Namespace: `com.xxf.view.gradient`
...
- 优先运行 `./gradlew :lib_view_gradient:compileDebugKotlin`；任务不存在时退回 `./gradlew :lib_view_gradient:assembleDebug`。
