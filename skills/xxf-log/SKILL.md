---
name: xxf-log
description: :lib_log 模块规则（core library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---
...
- 优先运行 `./gradlew :lib_log:compileDebugKotlin`；任务不存在时退回 `./gradlew :lib_log:assembleDebug`。
