---
name: xxf-viewpager
description: :optional:lib_viewPager 模块规则（optional library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---

# :optional:lib_viewPager

## Scope

- Gradle path: `:optional:lib_viewPager`
- Directory: `optional/lib_viewPager`
- Status: enabled in settings.gradle
- Type: optional library
- Namespace: `com.xxf.view.viewpager`
...
- 优先运行 `./gradlew :optional:lib_viewPager:compileDebugKotlin`；任务不存在时退回 `./gradlew :optional:lib_viewPager:assembleDebug`。
...
- `:optional:lib_viewPager:demo` at `optional/lib_viewPager/demo` is for verification/example only; do not expose it as an installable skill.
