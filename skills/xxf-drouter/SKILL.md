---
name: xxf-drouter
description: :optional:lib_drouter 模块规则（optional library，当前 settings.gradle 启用）。修改该模块、调用方、依赖或验证入口时使用。
---

# :optional:lib_drouter

## Scope

- Gradle path: `:optional:lib_drouter`
- Directory: `optional/lib_drouter`
- Status: enabled in settings.gradle
- Type: optional library
- Namespace: `com.xxf.drouter`
...
- 优先运行 `./gradlew :optional:lib_drouter:compileDebugKotlin`；任务不存在时退回 `./gradlew :optional:lib_drouter:assembleDebug`。
