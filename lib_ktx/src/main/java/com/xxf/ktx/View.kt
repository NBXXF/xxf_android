...
@Deprecated(
    "Use doOnDebouncingClick instead",
    ReplaceWith("doOnDebouncingClick(clickIntervals, isSharingIntervals, block)")
)
...
fun View.doOnDebouncingClick(
    ...
    setOnClickListener {
        val view =
            if (isSharingIntervals) context.findActivity()?.window?.decorView ?: this else this
...
@Deprecated(
    "Use doOnDebouncingLongClick instead",
    ReplaceWith("doOnDebouncingLongClick(clickIntervals, isSharingIntervals, block)")
)
...
/**
 * 扩展系统的setTag(int) 必须声明到xml 否则会报错 he key must be an application-specific
 */
@Suppress("UNCHECKED_CAST")
@RequiresApi(Build.VERSION_CODES.DONUT)
fun View.setTag(key: String, value: Any) {
    val tagMap =
        (this.getTag(R.id.tag_view_map_container_id) as? MutableMap<Any, Any>) ?: mutableMapOf()
    //插入新的
    tagMap[key] = value
    //放到系统的里面
    setTag(R.id.tag_view_map_container_id, tagMap)
}
 ...
 */
var <T : View> T.isEnabledUsingAlpha: Boolean
    get() {
